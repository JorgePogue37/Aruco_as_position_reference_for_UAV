...
#define IP_ADDRESS_MULTIROTOR	"127.0.0.1"
		...
		if(!ids.empty())
		{
			...
			for(i = 0; i < ids.size() && targetMarkedFound == 0; i++)
			{
				if(ids[i] == targetMarker)
				{
				    ...
				    Mat rotationMatrix;
				    Rodrigues(rvecs[i], rotationMatrix); // Convert rotation vector to matrix
				    
		            Mat transposedRotationMatrix;
		            transpose(rotationMatrix, transposedRotationMatrix);
				    
				    double yaw, pitch, roll;
				    yaw = atan2(transposedRotationMatrix.at<double>(1, 0), transposedRotationMatrix.at<double>(0, 0));
				    pitch = asin(-transposedRotationMatrix.at<double>(2, 0));
				    roll = atan2(transposedRotationMatrix.at<double>(2, 1), transposedRotationMatrix.at<double>(2, 2));
				    ...
				    markerRotation[0] = roll; //X
				    markerRotation[1] = pitch; //Y
				    markerRotation[2] = yaw;  //Z					
					...
					for(k = 0; k < 3; k++)
					{
						...
						dataPacketRobotPose.rot[k] = markerRotation[k];
